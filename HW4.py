...
class Human:
    def __init__(self, name="Human", job=None, home=None, car=None, gun=None):
        self.name = name
        self.job = job
        self.home = home
        self.car = car
    ...
    def live(self, day):
        ...
        if self.gun is None:
            self.get_gun()
            print(f"I bought a gun {self.gun}")
        if self.job is None:
            self.get_job()
...
class Pistol:
    def __init__(self, gun_list):
        self.choise = random.choice(list(gun_list))
        self.strength = gun_list[self.choise]["strength"]
